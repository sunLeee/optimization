# Plan: 팀 General Rules 완전 정의 시스템 구현 (v2)

- Plan ID: general-rules-toolkit-v2
- Spec: `.omc/specs/deep-interview-general-rules.md`
- Generated: 2026-03-18
- Supersedes: `general-rules-toolkit.md`
- Status: READY FOR REVIEW

---

## Architect ITERATE 피드백 반영 요약 (v1 → v2)

| 항목 | v1 결정 | v2 결정 |
|------|---------|---------|
| `python-patterns` 충돌 | Follow-up으로 미룸 | 같은 PR에서 deprecated 처리 + lint-fix 79자 수정 |
| AW 상세 위치 | `.claude/CLAUDE.md` 신규 생성 | `docs/design/ref/team-operations.md` 확장 (단일 진실 소스) |
| Stop hook 역할 | "이중 안전망" | 정보성 알림 (pre-commit이 실제 강제) |
...
| `.claude/skills/python-patterns/` | - | line-length=88 (Black 기준), 팀 규칙 미반영 |
| `.claude/skills/lint-fix/` | - | line 228에 88자 기준 |
| `.claude/skills/python-testing/` | - | 존재 |
| `.claude/skills/code-simplifier/` | - | 존재 |
| `.claude/agents/python-reviewer.md` | - | 존재 |
| `docs/design/ref/team-operations.md` | 91줄 | AW-001~005 일부, 확장 대상 |
...
**새로 만들어야 할 것**: Skills 9개 (+ python-patterns deprecated 처리), nested CLAUDE.md 3개,
pre-commit 설정, Stop hook 설정(정보성), output-styles 파일, 루트 CLAUDE.md 확장,
`docs/design/ref/team-operations.md` 확장

---

## Work Objectives

1. 루트 CLAUDE.md를 AW-001~010 요약 표 + LLM 행동지침 4개로 확장 (200줄 이하 유지)
2. `docs/design/ref/team-operations.md`를 AW-001~010 전체 상세 + 기술 어휘 규칙으로 확장
3. `.claude/skills/`에 convention 계열 skills 9개를 신규 생성
4. `python-patterns` skill에 deprecated 처리 + `lint-fix` 79자 수정
5. `agents/`, `libs/`, `apps/` 3곳에 nested CLAUDE.md 작성
6. `pre-commit` (실제 강제) + Stop hook (정보성 알림) 설정
7. `.claude/output-styles/data-driven-minds.md` 생성
...
- AW-001~AW-010 전부 어딘가에 존재 (루트 요약 표 + team-operations.md 상세)
- pre-commit: ruff `line-length=79`, mypy `--strict`, bandit 3종 모두
- 기술 용어 영어, 설명 한국어 규칙 준수
- `docs/references/github-issues/` 파일명 규칙 (`issue-{number}-{subject}.md`, kebab-case,
  30자 이하) 문서화
- `python-patterns/SKILL.md` frontmatter에 `deprecated: true` + `replaced_by: convention-python` 명시
- Stop hook의 역할이 "정보성 알림"임을 `.claude/settings.json` 또는 AW-010 설명에 명시
...
- `.claude/CLAUDE.md` 신규 생성 (v1에서 제거, team-operations.md로 대체)
- Stop hook에서 실제 ruff 실행 (정보성 echo만 허용)
...
### Option B: 우선순위별 순차 구현 (핵심 먼저) [채택]

**장점**
- 루트 CLAUDE.md는 단독 편집 후 줄 수 확인 가능
- 기반 파일(루트 CLAUDE.md, team-operations.md) 완성 후 나머지 파일이 이를 참조하며
  일관성 유지
...
구체적으로: Phase 1(기반 파일) 순차 → Phase 2(Skills, nested CLAUDE.md, python-patterns
deprecated) 병렬 → Phase 3(자동화 설정) 순차 → Phase 4(output-styles) 단독.
...
Phase 1: 기반 파일 확정 (순차, 다른 모든 것의 기준)
  └─ Step 1: 루트 CLAUDE.md 확장 (AW-001~010 요약 표, LLM 행동지침 4개, 200줄 이하)
  └─ Step 2: docs/design/ref/team-operations.md 확장 (AW 상세 + 기술 어휘 규칙)

Phase 2: Skills + nested CLAUDE.md + deprecated 처리 (병렬 가능, Phase 1 완료 후)
  ├─ [병렬 A] Skills 9개 생성
  ...
  └─ [병렬 C] python-patterns deprecated 처리 + lint-fix 79자 수정

Phase 3: 자동화 설정 (순차, Skills 완료 후 설정 파일이 skill 이름 참조)
  └─ Step 3: .pre-commit-config.yaml 생성 (실제 강제)
  └─ Step 4: settings.json Stop hook 등록 (정보성 알림)
...
v1에서는 AW 상세 내용을 `.claude/CLAUDE.md`에 위임하려 했으나, v2에서는
`docs/design/ref/team-operations.md`에 위임한다. 따라서 루트 CLAUDE.md에는
AW-006~010를 **요약 표 1줄씩**으로만 기재한다.

| 현재 | 추가 | 예상 최종 |
|------|------|----------|
| 177줄 | AW-006~010 표 약 12줄 + LLM 행동지침 약 12줄 + 필수 스킬 3행 추가 약 3줄 | 약 204줄 |

**줄 수 전략**: 현재 CLAUDE.md에서 중복 내용 또는 team-operations.md 참조로 대체 가능한
섹션을 6줄 이상 축약하여 200줄 이하를 달성한다. 구체적 대상: `## Git Workflow` 섹션의
상세 설명 중 team-operations.md에 이미 있는 내용 압축.

**추가 내용 (섹션 제목 수준)**

1. `## Team Operations` 섹션에 AW-006~010 추가 (요약 표 형태)
   - AW-006: 설계문서 먼저, 코드 나중
   - AW-007: 모든 구현 `/ralph` 사용
   - AW-008: ralplan 종료조건 10개 이상
   - AW-009: 되돌리기 어려운 결정 = ADR 먼저
   - AW-010: pre-commit 강제 + Stop hook 알림 (상세: team-operations.md 참조)
2. `## LLM 행동지침` 섹션 신규 추가 (각 항목 3줄 이하로 압축)
   - Think Before Coding
   - Simplicity First
   - Surgical Changes
...
**수락 기준**
- `wc -l CLAUDE.md | awk '{print ($1 <= 200)}'` 출력이 1
- `grep -o "AW-0[0-9][0-9]" CLAUDE.md | sort -u | wc -l` 출력이 10 이상
- `grep "LLM 행동지침" CLAUDE.md` 존재
- `grep "team-operations" CLAUDE.md` 존재 (상세 위임 링크)

---

### Step 2: `docs/design/ref/team-operations.md` 확장

**대상 파일**: `docs/design/ref/team-operations.md` (현재 91줄, 확장 후 200줄 이하)

**배경**: v1에서 `.claude/CLAUDE.md` 신규 생성 계획이었으나, Architect 피드백에 따라
기존 패턴을 유지하고 `team-operations.md`를 단일 진실 소스로 확장한다.
기존 AW-001~005 내용을 유지하면서 AW-006~010을 추가하고, 기술 어휘 규칙과
명명 규칙을 이 파일에 통합한다.
...
## GitHub Issues md화 규칙 (신규 추가)
  파일명: issue-{number}-{subject}.md
  저장 경로: docs/references/github-issues/
  생성 방법: gh api 기반
```

**AW-010 설명 (pre-commit vs Stop hook 역할 명확화)**

```
AW-010: pre-commit 안전망 + Stop hook 알림

역할 구분:
- pre-commit (실제 강제): git commit 시 ruff(79자), mypy --strict, bandit 자동 실행.
  실패 시 커밋 차단. --no-verify 사용 금지.
- Stop hook (정보성 알림): Claude Code 응답 완료 후 /check-python-style 실행 안내
  메시지를 출력한다. 실제 ruff를 실행하지 않으며, 개발 중 자기 점검 트리거 역할.

표현: "pre-commit 안전망 + Stop hook 알림"으로 통일 (v1의 "이중 안전망" 폐기)
```

**수락 기준**
- `wc -l docs/design/ref/team-operations.md | awk '{print ($1 <= 200)}'` 출력이 1
- `grep -o "AW-0[0-9][0-9]" docs/design/ref/team-operations.md | sort -u | wc -l` 출력이 10 이상
- `grep "kebab-case" docs/design/ref/team-operations.md` 존재
- `grep "snake_case" docs/design/ref/team-operations.md` 존재
- `grep "issue-{number}" docs/design/ref/team-operations.md` 존재
- `grep "정보성" docs/design/ref/team-operations.md` 존재 (Stop hook 역할 명시)
- `grep "no-verify" docs/design/ref/team-operations.md` 존재 (금지 명시)

---

### Step 3: `python-patterns` deprecated 처리 + `lint-fix` 79자 수정 (병렬 C)

**배경**: 기존 `python-patterns/SKILL.md`는 line-length=88 (Black 기준)을 사용하고,
`lint-fix/SKILL.md` line 228도 88자 기준이다. 팀 기준인 79자 (`convention-python`)와
충돌하므로 같은 PR에서 해소한다.

#### 3-1. `python-patterns/SKILL.md` deprecated 처리

frontmatter에 deprecated 표시 추가:
...
deprecated: true
replaced_by: convention-python
deprecated_reason: >
  팀 기준 line-length=79 (ruff)으로 통일. 기존 88자(Black) 기준과 충돌.
  convention-python skill로 대체되었습니다.
---
```
...
```markdown
> **DEPRECATED**: 이 skill은 더 이상 사용하지 않습니다.
> `convention-python` skill로 대체되었습니다. (line-length: 88 → 79)
```

**수락 기준**
- `grep "deprecated: true" .claude/skills/python-patterns/SKILL.md` 존재
- `grep "replaced_by: convention-python" .claude/skills/python-patterns/SKILL.md` 존재
- `grep "DEPRECATED" .claude/skills/python-patterns/SKILL.md` 존재

#### 3-2. `lint-fix/SKILL.md` 79자 수정

- Line 228 (`88` 또는 `line-length = 88`)을 `79` 또는 `line-length = 79`로 수정
- 파일 내 다른 88 기준 언급도 79로 일괄 수정 (grep으로 확인 후)

**수락 기준**
- `grep "88" .claude/skills/lint-fix/SKILL.md` 출력 없음 (79로 전환 완료)
- `grep "79" .claude/skills/lint-fix/SKILL.md` 존재

---

### Step 4: Skills 9개 생성 (병렬 A)
...
**`python-patterns` 대체 선언** 포함:

```
## Supersedes
  - .claude/skills/python-patterns/ (deprecated: true)
  - 이전 Black 88자 기준은 이 skill의 79자 기준으로 대체됨
```

수락 기준:
- `grep "79자" .claude/skills/convention-python/SKILL.md` 존재
- `grep "Logics" .claude/skills/convention-python/SKILL.md` 존재
- `grep "iterrows" .claude/skills/convention-python/SKILL.md` 존재
- `grep "python-patterns" .claude/skills/convention-python/SKILL.md` 존재

#### 4-2. `convention-commit`
...
수락 기준:
- `grep "Co-Authored-By" .claude/skills/convention-commit/SKILL.md` 존재
- `grep "kebab-case\|{type}/{issue" .claude/skills/convention-commit/SKILL.md` 존재

#### 4-3. `convention-design`
...
수락 기준:
- `grep "SRP\|KISS\|DRY\|YAGNI" .claude/skills/convention-design/SKILL.md` 4개 모두 존재

#### 4-4. `convention-design-patterns`
...
수락 기준:
- `grep -c "상황 1\|상황 2" .claude/skills/convention-design-patterns/SKILL.md` 출력 8 이상

#### 4-5. `convention-folder-structure`
...
수락 기준:
- `grep -c "상황 1\|상황 2" .claude/skills/convention-folder-structure/SKILL.md` 출력 8 이상

#### 4-6. `convention-logging`
...
수락 기준:
- `grep "structlog\|TRACE\|JSON" .claude/skills/convention-logging/SKILL.md` 3개 존재
...
수락 기준:
- `grep "되돌리기 어려운\|되돌리기 쉬운" .claude/skills/convention-adr/SKILL.md` 존재
...
수락 기준:
- `grep "Rule of Small\|Rule of Design" .claude/skills/convention-pr/SKILL.md` 존재
- `grep "check-design-doc" .claude/skills/convention-pr/SKILL.md` 존재

#### 4-9. `check-design-doc`
...
수락 기준:
- `grep "check-design-doc\|check_design_doc" .claude/skills/check-design-doc/SKILL.md` 존재

---

### Step 5: Nested CLAUDE.md 3개 생성 (병렬 B)

**참고**: v2에서는 `.claude/CLAUDE.md`를 생성하지 않으므로 nested CLAUDE.md는 3개다.

#### 5-1. `agents/CLAUDE.md`
...
수락 기준:
- `wc -l agents/CLAUDE.md | awk '{print ($1 <= 200)}'` 출력 1
...
수락 기준:
- `wc -l libs/CLAUDE.md | awk '{print ($1 <= 200)}'` 출력 1
...
수락 기준:
- `wc -l apps/CLAUDE.md | awk '{print ($1 <= 200)}'` 출력 1
- `grep "get_.*_path" apps/CLAUDE.md` 존재

---

### Step 6: pre-commit 설정 (Phase 3)

**역할**: pre-commit은 실제 강제 도구다. git commit 시 자동 실행되며 실패 시 커밋을 차단한다.
...
**중요 규칙 문서화**: `--no-verify` 사용 금지를 `docs/design/ref/team-operations.md`
AW-010에 명시 (v1의 `.claude/CLAUDE.md` 위치에서 변경)
...
### Step 7: Stop hook 등록 (Phase 3, 정보성 알림)

**역할**: Stop hook은 정보성 알림 도구다. Claude Code 응답 완료 후 개발자에게
`/check-python-style` 자기 점검을 상기시키는 메시지를 출력한다.
실제 ruff를 실행하지 않으며, 커밋을 차단하지 않는다.
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo '[Stop Hook] 알림: /check-python-style 자기 점검을 권장합니다. (실제 강제는 pre-commit)'"
          }
        ]
      }
    ]
...
**"이중 안전망" → "pre-commit 안전망 + Stop hook 알림"으로 표현 수정**:
AW-010 설명 및 `docs/design/ref/team-operations.md`에서 "이중 안전망" 표현을 사용하지 않는다.

수락 기준:
- `grep "Stop" .claude/settings.json` 존재
- `grep "echo" .claude/settings.json` 존재 (실제 ruff가 아닌 echo만)
- `grep "ruff\|mypy\|bandit" .claude/settings.json` 출력 없음 (Stop hook에서 실제 실행 금지)

---

### Step 8: output-styles 파일 (Phase 4, 독립 실행 가능)
...
수락 기준:
- `test -s .claude/output-styles/data-driven-minds.md && echo "OK"` 출력 OK
...
(v1 대비 변경: 조건 2, 8이 team-operations.md 기준으로 교체, 조건 16~17 신규 추가)
...
**Decision**: Option B (우선순위별 순차+병렬 혼합)를 선택한다. Architect ITERATE 피드백을
반영하여 `.claude/CLAUDE.md` 신규 생성 대신 `docs/design/ref/team-operations.md` 확장을
채택하고, Stop hook은 정보성 알림으로 명확히 재정의하며, `python-patterns`를 같은 PR에서
deprecated 처리한다.

**Drivers**:
1. 루트 CLAUDE.md 200줄 제약이 모든 하위 파일에 영향 — 기반 파일 우선 확정 필요
2. `docs/design/ref/team-operations.md`가 이미 AW-001~005를 담고 있어 단일 진실 소스
   유지가 자연스럽다 (기존 패턴 존중)
3. Stop hook과 pre-commit의 역할 혼동 방지 — 명확한 역할 분리가 팀 신뢰성 향상

**Alternatives Considered**:
- Option A (전체 병렬): 루트 CLAUDE.md 동시 편집 충돌 위험으로 기각
- `.claude/CLAUDE.md` 신규 생성 (v1): AW 중복 위험 + 새 파일 위치 학습 부담으로 기각
- Stop hook에서 실제 ruff 실행: pre-commit과 역할 중복, 응답 속도 저하 우려로 기각

**Why Chosen**: 기반 파일 확정 → 나머지 병렬의 혼합 전략이 품질과 속도를 동시에 달성.
기존 `team-operations.md` 활용은 새 파일 위치 학습 없이 팀이 자연스럽게 규칙을 찾을 수 있게 한다.

**Consequences**:
- 긍정: 루트 CLAUDE.md 줄 수 초과 위험 제거, 단계별 verifier 검증 가능,
  `.claude/CLAUDE.md` 신규 파일 미생성으로 파일 위치 혼란 없음,
  Stop hook 역할이 명확해 팀원 혼동 없음
- 부정: Phase 1 완료 전 Phase 2 착수 불가 (대기 시간 소량 발생),
  team-operations.md가 91 → 200줄 근접으로 향후 추가 여유 감소

**Follow-ups**:
- `docs/design/ref/team-operations.md` 200줄 한계 도달 시 분리 기준 ADR 작성 필요
- convention-python과 python-patterns 모두 사용하는 팀원에게 deprecated 안내 메시지 확인
...
- AW-001~010 전부 문서화됨 (루트 요약 + team-operations.md 상세)
- Skills 9개 모두 생성, 각 skill 상황 예시 2개 이상
- `python-patterns` deprecated 처리 완료 + `lint-fix` 79자 수정 완료
- Nested CLAUDE.md 3개 모두 200줄 이하 (`.claude/CLAUDE.md` 미생성)
- pre-commit: ruff 79자, mypy strict, bandit 3종 설정 (실제 강제)
- Stop hook: 정보성 echo 메시지만 (실제 ruff 실행 없음)
- `docs/design/ref/team-operations.md`: AW 상세 + 기술 어휘 규칙 포함
