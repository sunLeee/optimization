...
# Codex CLI (omc-teams codex worker 사용 시 필수)
npm install -g @openai/codex
# 설치 확인
codex --version

# Gemini CLI (omc-teams gemini worker 사용 시 필수)
npm install -g @google/gemini-cli
# 사용할 모델 설정 (고성능 사고 모델 권장)
export GEMINI_MODEL=gemini-2.0-flash-thinking-exp
# ~/.zshrc 또는 ~/.bashrc에 영구 설정
echo 'export GEMINI_MODEL=gemini-2.0-flash-thinking-exp' >> ~/.zshrc
```
